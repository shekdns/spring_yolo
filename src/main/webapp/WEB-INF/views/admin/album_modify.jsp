...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">album Modify</h1>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
...
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			...
			<div class="panel-heading">album Modify</div>
			<!-- /.panel-heading -->
			<div class="panel-body">

				<form role="form" action="/admin/album_modify" method="post"
       	...
       	-->
					...
					<c:choose>
						<c:when test="${not empty album.album_img_path}">
							<input type="hidden" name="album_img_path" value="${album.album_img_path}">
						</c:when>
						<c:otherwise>
					...
					<div class="form-group">
						<label>앨범 이름</label> <input class="form-control"
							name='album_name' value='<c:out value="${album.album_name }"/>'>
					</div>
					...
					<div class="form-group">
						<label>장르</label> <input class="form-control" name='genre'
							value='<c:out value="${album.genre}"/>'>
					</div>

					<div class="form-group">
						<label>타입</label> <input class="form-control" name='album_type'
							value='<c:out value="${album.album_type}"/>'>
					</div>
					...
					<div class="form-group">
						...
						<c:if test="${not empty album.album_img_path}">
							<a href="/resources/upload/${album.album_img_path}" target="_blank"><img
								src="/resources/upload/${album.album_img_path}" id="thumb_"></a>
							<script>
								document.getElementById('thumb_').src = "/resources/upload/"
										+ getThumbFileName('${album.album_img_path}');
							</script>
						</c:if>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('button').on("click", function(e) {
			...
			if (operation === 'remove') {
				formObj.attr("action", "/admin/album_remove");

			} else if (operation === 'list') {
				//move to list
				formObj.attr("action", "/admin/album").attr("method", "get");
