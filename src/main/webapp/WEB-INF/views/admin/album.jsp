			...
			<div class="panel-body">
				<table class="table table-striped table-bordered table-hover">
					...
					<c:forEach items="${album}" var="album">
						<tr>
							...
							<td><c:out value="${album.album_type}" /></td>
							<td><c:out value="${album.genre}" /></td>
						</tr>
						  <script>
                 		   document.getElementById('thumb_').src="/resources/upload/" + getThumbFileName('${album_img_path}');
               			  </script>
					</c:forEach>
				...
				<div class='row'>
					<div class="col-lg-12">
						...
						<form id='searchForm' action="/admin/album" method='get'>
							<select name='type'>
								<option value=""
								...
								<option value="TC"
									<c:out value="${pageMaker.cri.type eq 'TC'?'selected':''}"/>>앨범
									or 가수</option>
								<option value="TW"
									<c:out value="${pageMaker.cri.type eq 'TW'?'selected':''}"/>>앨범
									or 장르</option>
								<option value="TWC"
									<c:out value="${pageMaker.cri.type eq 'TWC'?'selected':''}"/>>앨범
									or 가수 or 장르</option>
				...
				<!-- 페이징 Form 시작 -->
				<form id='actionForm' action="/admin/album" method='get'>
			...
			.ready(
					function() {
						...
						$("#regBtn").on("click", function() {
							self.location = "/admin/album_register";
						});
						-->
